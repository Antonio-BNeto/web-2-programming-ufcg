...
interface StatCard { label: string; value: string | number; icon: string; href: string; }

const quickActions = [
  { href: '/my-items', icon: '📦', label: 'Meus Anúncios', description: 'Gerencie seus itens à venda' },
  { href: '/sales', icon: '🛒', label: 'Minhas Vendas', description: 'Acompanhe suas transações' },
  { href: '/payments', icon: '💳', label: 'Pagamentos', description: 'Registre e veja pagamentos' },
  { href: '/payment-methods', icon: '🏦', label: 'Formas de Pagamento', description: 'PIX, cartão e conta bancária' },
  { href: '/profile', icon: '👤', label: 'Meu Perfil', description: 'Atualize seus dados pessoais' },
];
    ...
    async function load() {
      ...
      setStats([
        ...
        },
        {
          ...
          value: methodsRes.status === 'fulfilled' ? methodsRes.value.length : '-',
          icon: '🏦',
