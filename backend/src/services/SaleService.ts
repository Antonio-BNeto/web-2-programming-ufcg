...
class SaleService {
    ...
    public async getSalesPaginated(
        userId: number,
        page: number,
        limit: number
    ) {
    ...
    public async getAllSalesPaginatedAdmin(
        page: number,
        limit: number
    ) {
        return await paginate(Sale, page, limit, {
    ...
    public async createSale(
        ...
        try {
            ...
            for (const itemRef of itemsData) {
                ...
                if (!item) {
                    throw new Error(`Item ID ${itemRef.itemId} não encontrado.`);
                }

                if (item.quantity < itemRef.quantity) {
                    throw new Error(`Estoque insuficiente: ${item.name}`);
                }

                const price = Number(item.price);

                calculatedTotal += price * itemRef.quantity;
            ...
            for (const itemData of itemsToProcess) {
                ...
                await itemData.model.update(
                    {
                        quantity:
                            itemData.model.quantity - itemData.quantity
                    },
                    { transaction }
                );
            }
